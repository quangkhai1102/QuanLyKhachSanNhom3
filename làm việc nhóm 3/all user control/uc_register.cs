...
namespace làm_việc_nhóm_3.all_user_control
{
    public partial class uc_register : UserControl
    {
        ...
        {
            // Load tất cả loại giường vào ComboBox khi form được tải
            query = "SELECT DISTINCT Giuong FROM Phong WHERE Dadat = 'NO'";
            setComboBox(query, cb_bed);

            // Load tất cả loại phòng vào ComboBox khi form được tải
            query = "SELECT DISTINCT Loaiphong FROM Phong WHERE Dadat = 'NO'";
        ...
        {
            // Khi chọn loại giường, hiển thị tất cả các loại phòng
            cb_room.Items.Clear();
            cb_nroom.Items.Clear();
            txt_price.Clear();

            // Truy vấn để hiển thị tất cả các loại phòng mà không lọc theo loại giường
            query = "SELECT DISTINCT Loaiphong FROM Phong WHERE Dadat = 'NO'";
            setComboBox(query, cb_room);
        }
        ...
        {
            // Khi chọn loại phòng, load danh sách phòng trống
            cb_nroom.Items.Clear();
            txt_price.Clear();
        ...
        {
            // Khi chọn phòng, hiển thị giá phòng và lưu ID phòng
